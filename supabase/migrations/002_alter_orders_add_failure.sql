...
serve(async (req)=>{
  console.log("=== VERIFY PAYMENT FUNCTION CALLED ===", new Date().toISOString());
  ...
  try {
    ...
    const firebaseVerifyUrl = `https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=${Deno.env.get("FIREBASE_WEB_API_KEY")}`;
    let firebaseUserId;
    try {
      ...
      if (!firebaseResponse.ok) {
        console.error("Firebase token verification failed:", await firebaseResponse.text());
        return new Response(JSON.stringify({
    ...
    const { data: order, error: orderError } = await supabaseClient.from("orders").select("*").eq("razorpay_order_id", orderId).eq("user_id", userId).single();
    if (orderError || !order) {
    ...
    const isValidSignature = await verifySignature(orderId, paymentId, signature, razorpaySecret);
    if (!isValidSignature) {
    ...
    const { data: existingUser, error: userCheckError } = await supabaseClient.from("users").select("id, credits").eq("id", userId).single();
    ...
    const { data: updatedUser, error: verifyError } = await supabaseClient.from("users").select("id, credits").eq("id", userId).single();
    console.log("Credits verification:", {
      updatedUser,
      verifyError
    });
